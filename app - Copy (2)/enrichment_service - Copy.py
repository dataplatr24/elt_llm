...
def get_table_metadata(connection, catalog: str, schema: str, table: str) -> Dict:
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            col_comment = row[2] if len(row._values if hasattr(row, '_values') else row) > 2 else None
...
def call_databricks_llm(prompt: str, access_token: str, server_hostname: str) -> str:
    ...
    try:
        ...
        print(f"DEBUG: Response: {response.text if 'response' in locals() else 'No response'}")
        raise
